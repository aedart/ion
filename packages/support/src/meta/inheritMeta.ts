...
export function inheritMeta()
{
    ...
    ): void
    {
        ...
        if (context.kind === 'class') {
            throw new TypeError(
                `Invalid target class "${
                    getNameOrDesc(target as ConstructorLike)
                } for inheritMeta(). Only static class members can inherit meta!"`,
            );
        }
        ...
        if (!context.static) {
            throw new TypeError(
                `Invalid target ${context.kind} "${
                    String(context.name)
                }" for inheritMeta(). Member is not static!`,
            );
        }
