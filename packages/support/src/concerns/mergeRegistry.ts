 ...
 */
export function mergeRegistry(target: any, concern: ConcernConstructor, targetRegistry: Set<ConcernConstructor>): void
{
    ...
    for (let i: number = 0, limit: number = entries.length; i < limit; i++) {
        ...
        if (targetRegistry.has(nestedConcern)) {
            throw new AlreadyAppliedError(target, nestedConcern, `Nested concern ${nestedConcern.name} (from ${concern.name}) is already applied.`);
        }
