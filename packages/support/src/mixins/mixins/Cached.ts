 ...
 * @returns {Mixin}
 */
export const Cached = function(mixin: Mixin): Mixin
{
    ...
    return wrap(mixin, (superclass: object) => {
        ...
        let cachedApplications: WeakMap<Mixin, object> | undefined = Reflect.has(superclass, CACHED_APPLICATIONS)
            ? superclass[CACHED_APPLICATIONS]
            : undefined;

        if (cachedApplications === undefined) {
            cachedApplications = new WeakMap<Mixin, object>();
