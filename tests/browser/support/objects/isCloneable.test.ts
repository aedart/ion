    ...
    describe('isCloneable', () => {
        ...
        test('can determine if is cloneable', () => {
            ...
            const dataSet = [
                ...
                { value: { [CLONE]: () => this }, expected: true, name: 'Object with clone function' },
                { value: new A(), expected: true, name: 'Object that implements interface (clone function)' },
            ];

            for (const data of dataSet) {
                // @ts-ignore
                expect(isCloneable(data.value), `${data.name} was expected to ${data.expected.toString()}`)
                    .toBe(data.expected);
            }
        });
